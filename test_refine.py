...
from refine.runner import LLMRefineRunner,DEFAULT_ITERATION_COUNT
import asyncio
import json
import time
...
async def test_refine_different_iterations():
    ...
    print(f"\n🔄 测试迭代次数: {DEFAULT_ITERATION_COUNT}")
    print("-" * 40)
    
    start_time = time.time()
    result = await runner.run_async(question, DEFAULT_ITERATION_COUNT, knowledge)
    end_time = time.time()
