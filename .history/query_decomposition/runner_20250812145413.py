...
class QueryDecompositionRunner(LLMCallMixin):
    ...
    async def run_async(self, question: str, context: list[str], retrieval_func=None) -> str:
        """
...
"""
    ...
    async def _process_sub_query_async(self, sub_query: Dict[str, Any], context: list[str], 
                                      retrieval_func=None, index: int = 1) -> Dict[str, Any]:
        """异步处理单个子问题"""
        try:
            # 如果提供了检索函数，使用它检索相关上下文
            if retrieval_func:
