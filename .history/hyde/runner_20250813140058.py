...
class HydeRunner(LLMCallMixin):
    ...
    def generate_hypothetical_answer(self, question: str, prompt_type: Optional[str] = None) -> str:
        ...
        """
        # 检查缓存
        cache_key = f"{question}:{prompt_type}"
        ...
        # 缓存结果
        self.hyde_cache[cache_key] = hypothetical_answer
    ...
    async def generate_hypothetical_answer_async(self, question: str, prompt_type: Optional[str] = None) -> str:
        ...
        print(f"正在异步生成假设性答案 (类型: {prompt_type})...")
        
        # 调用LLM生成假设性答案
        messages = self.create_messages(user_content=hyde_prompt)
        hypothetical_answer = await self.call_llm_async(messages)
    ...
    async def generate_hypothetical_answer_stream(self, question: str, prompt_type: Optional[str] = None) -> str:
        """
        流式生成假设性答案
        ...
        print(f"正在流式生成假设性答案 (类型: {prompt_type})...")
        print("生成过程:")
    ...
    async def run_with_retrieval_async(self, question: str, retrieval_func, prompt_type: Optional[str] = None,
                                     top_k: int = 5) -> Dict[str, Any]:
        """
        异步版本的完整Hyde策略执行
