...
def parse_classification_result(classification_response: str) -> tuple:
    """
    解析LLM返回的分类结果
    
    Args:
        classification_response: LLM返回的分类响应
        
    Returns:
        tuple: (分类结果, 分类理由)
    """
    lines = classification_response.strip().split('\n')
    category = "enhanced"  # 默认分类
    reason = "无法确定具体分类"
    
    for line in lines:
        line = line.strip()
        if line.startswith("分类结果："):
            category_text = line.replace("分类结果：", "").strip()
            # 提取方括号内的内容
            if '[' in category_text and ']' in category_text:
                category = category_text.split('[')[1].split(']')[0].strip()
            else:
                # 如果没有方括号，直接取文本
                category = category_text
        elif line.startswith("分类理由："):
            reason = line.replace("分类理由：", "").strip()
    
    # 验证分类结果是否有效
    valid_categories = ["technical", "business", "academic", "enhanced"]
